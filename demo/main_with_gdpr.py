...
import spacy
import json
from demo.gdpr_compliance import export_user_data, delete_user_data, apply_data_retention_policies
...
@app.get("/api/gdpr/export/{user_id}", dependencies=[Depends(verify_api_key)])
async def gdpr_export(user_id: str):
    """
    Exports all data for a given user, in compliance with GDPR's Right of Access.
    """
    exported_data = export_user_data(user_id)
    if not exported_data or exported_data == "{}":
        raise HTTPException(status_code=404, detail="User not found.")
    return {"user_id": user_id, "data": json.loads(exported_data)}

@app.delete("/api/gdpr/delete/{user_id}", dependencies=[Depends(verify_api_key)])
async def gdpr_delete(user_id: str):
    """
    Deletes all data for a given user, in compliance with GDPR's Right to Erasure.
    """
    if not delete_user_data(user_id):
        raise HTTPException(status_code=404, detail="User not found.")
    return {"status": "success", "message": f"Data for user {user_id} has been deleted."}

@app.post("/api/gdpr/apply-retention", dependencies=[Depends(verify_api_key)])
async def gdpr_apply_retention():
    """
    Applies data retention policies to the entire user database.
    """
    apply_data_retention_policies()
    return {"status": "success", "message": "Data retention policies applied."}
